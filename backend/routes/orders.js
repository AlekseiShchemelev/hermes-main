...
router.get("/stats", async (req, res) => {
  try {
    ...
    const totalValueResult = await query(
      "SELECT COALESCE(SUM(total_price), 0) as sum FROM orders WHERE status = $1",
      ["active"]
    );
...
router.post("/", async (req, res) => {
  try {
    ...
    // if (!order_number || !bottom_number || !date || !material || !thickness) {
    //   return res.status(400).json({ error: "Missing required fields" });
    // }

    const result = await query(
      `
      INSERT INTO orders (order_number, bottom_number, date, material, thickness)
      VALUES ($1, $2, $3, $4, $5)
      RETURNING *
    `,
      [order_number, bottom_number, date, material, thickness]
    );
    ...
    if (error.code === "23505") {
      ...
      res
        .status(400)
        .json({ error: "Order number or bottom number already exists" });
    } else {
...
router.post("/export/csv", async (req, res) => {
  try {
    ...
    const result = await query(
      `SELECT * FROM orders ${whereClause} ORDER BY date DESC`,
      params
    );
      ...
      .on("end", async () => {
        try {
          ...
          if (orders.length === 0) {
            return res
              .status(400)
              .json({ error: "No valid orders found in CSV", errors });
          }
